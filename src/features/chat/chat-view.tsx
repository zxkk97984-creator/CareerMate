  ...
  return (
    <section className="rounded-lg border border-slate-200 bg-white shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-5 py-4">
        <div>
          <h2 className="text-base font-semibold text-slate-950">CareerMate 智能聊天</h2>
          <p className="mt-1 text-xs text-slate-500">当前页面内保持多轮上下文，刷新后自动清空。</p>
        </div>
        {action}
      </div>
  ...
  return (
    <div className="space-y-2" aria-label="聊天上下文">
      <div className="flex flex-wrap gap-2">
        {badges.map((badge) => (
          <span key={badge} className="rounded-full bg-slate-100 px-3 py-1 text-xs text-slate-600">
            {badge}
          </span>
        ))}
      </div>
      {context?.knowledgeSources.length ? (
        <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500">
          ...
          {context.knowledgeSources.map((source) => (
            <span key={source} className="rounded border border-indigo-100 bg-indigo-50 px-2 py-1 text-indigo-700">
              {source}
            </span>
          ))}
        </div>
  ...
  return (
    ...
    >
      <div className="space-y-4">
        ...
        <div className="min-h-56 space-y-3 rounded-lg bg-slate-50 p-4" aria-label="聊天记录">
          {messages.length === 0 ? (
        ...
        <div className="space-y-3">
          <textarea
            aria-label="聊天问题"
            className="min-h-28 w-full rounded-md border border-slate-200 p-3 text-sm leading-6 outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100"
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <button
              ...
              onClick={send}
              className="h-10 rounded-md bg-slate-950 px-5 text-sm font-semibold text-white hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-50"
            >
              {streaming ? "生成中..." : "发送"}
            </button>
          </div>
