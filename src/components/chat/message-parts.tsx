        ...
        {items.items.map((c, i) => {
          ...
          return (
            <li key={i} className="citation-item">
              <span className={"citation-label label-" + tone}>
                <Icon size={12} />
                {c.label}
              </span>
              {c.url ? (
  ...
  if (plan && ["generating", "processing"].includes(plan.status)) {
    return (
      <div className="parts-card parts-card-plan parts-card-generating" role="status">
        ...
        {generationError ? <span className="parts-card-hint text-[var(--cm-danger)]">{generationError}</span> : null}
      </div>
    );
  }
