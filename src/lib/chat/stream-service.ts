...
async function handleStatefulStream(
  ...
  // 知识库预检索（hybrid）：优先依据知识库回答，缺口再由 Agent 联网补充
  let knowledgeItems: RetrievalItem[] = [];
  if (!agenticV2 && config.retrievalMode === "hybrid") {
    const intent = classifyCareerChatIntent(message);
    if (intent) {
      const query = [realProfile?.targetRoleLabel, message].filter(Boolean).join(" ");
      try {
        const retrieval = await retrieveWithTbox(
          { datasetKey: intent, query, limit: 3 },
          { config },
        );
        if (retrieval.meta.actualMode === "api") {
          knowledgeItems = retrieval.data.items.filter(
            (item) => item.content.trim() && item.source.trim(),
          );
        }
      } catch {
        // 检索失败不阻塞对话，交由 Agent 自行检索知识库
      }
    }
  }
  // 三种互斥上下文模式
    ...
    async start(controller) {
      ...
      try {
        ...
        if (agenticV2) {
          ...
          question = buildEnhancedQuestion(message, agentContext as unknown as Record<string, unknown>, knowledgeItems);
          history = undefined;
          context = undefined;
        }
