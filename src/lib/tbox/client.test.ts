...
describe("tbox client request contract", () => {
  ...
  it("enables search_engine when global flag on and policy allowed", async () => {
    const fetchFn = vi.fn(async (_url: URL | RequestInfo, _init?: RequestInit) => {
      void _url; void _init;
      return new Response("{}", { status: 200 });
    });
    await consumeChatResponse(
      { question: "hello", userId: "user-1", searchPolicy: "allowed" },
      false,
      { config: { ...baseConfig, searchEngine: true }, fetchImpl: fetchFn as typeof fetch },
      async () => undefined,
    );
    const body = JSON.parse(String(fetchFn.mock.calls[0]![1]?.body));
    expect(body.search_engine).toBe(true);
  });

  it("keeps search_engine off when per-turn policy is off", async () => {
    const fetchFn = vi.fn(async (_url: URL | RequestInfo, _init?: RequestInit) => {
      void _url; void _init;
      return new Response("{}", { status: 200 });
    });
    await consumeChatResponse(
      { question: "hello", userId: "user-1", searchPolicy: "off" },
      false,
      { config: { ...baseConfig, searchEngine: true }, fetchImpl: fetchFn as typeof fetch },
      async () => undefined,
    );
    const body = JSON.parse(String(fetchFn.mock.calls[0]![1]?.body));
    expect(body.search_engine).toBe(false);
  });
