...
/**
 * Search is enabled when the global flag is on, unless the per-turn
 * policy explicitly opts out with "off".
 */
function resolveSearchEnabled(deps: TboxDependencies, input: ChatInput): boolean {
  const globalOn = deps.config.searchEngine === true;
  const perTurnOff = (input as ChatInput & { searchPolicy?: string }).searchPolicy === "off";
  return globalOn && !perTurnOff;
}
