...
describe("Tbox fallback adapter", () => {
  it("reports API success metadata and follows the chat request protocol", async () => {
    ...
    expect(result).toEqual({
      data: { text: "API answer", conversationId: "conversation-2", citations: [], warnings: [], structured: undefined, toolCalls: [] },
      meta: {
        requestedMode: "api",
        actualMode: "api",
        degraded: false,
  ...
  it.each([
    ...
    const result = await generateStructuredWithTbox({
      ...
      fetchImpl: vi.fn(async () =>
        sseResponse([
          { event: "conversation.chat.created", data: { conversation_id: "conversation-1" } },
          { event: "conversation.message.delta", data: { type: "answer", content_type: "text", content: answer } },
