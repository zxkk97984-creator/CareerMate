...
11. **动态背景「星野」**(2026-09-03 用户追加需求,替换原网格视差背景):参考 Linear/Vercel/Stripe 式多光斑 morph 与星座粒子做法,五光斑极光(天蓝/青/珊瑚三色阶,22-36s 反向 morph,外壳分层深景视差)+ 粒子星座(canvas 110/50 点,开场即全屏散布、近距连线、粒子间斥力防缩团、光标 220px 内粒子绕光标环形流转,平衡环 90px)+ 流星(每 9-15 秒珊瑚色带尾迹)+ 光标光尾(1.2s 渐隐轨迹)+ 粒子色相随时间 ±10° 流转 + 双色透镜(蓝芯珊瑚缘)+ 胶片颗粒。登录页用 calm 变体(粒子减半、无流星无光尾、光斑节奏 ×1.6)。视口外与标签页隐藏暂停 rAF,dpr 上限 2,reduced-motion 全静止,触屏不挂光标跟随。原 `interactive-background.tsx` 及其测试已删除,新组件 `fluid-background.tsx`(落地页+登录页两处使用)。
